...
\begin{theorem}
    \textbf{4.12. (\emph{Sylow})} Let \(P\leq G\) $p$-Sylow and  \(n_p := \#\{H \leq G : H \text{ is }p\text{-Sylow}\}\), then
    \vspace{-0.2cm}
    \begin{enumerate}[label = \roman*., left = 0.2cm]
        \item \(\forall H\leq G\) $p$-Sylow, \(\exists g \in G : H = P^g\). \hfill ii. \( n_p \equiv 1 \ (\operatorname{mod}\ p)\) and \(n_p \mid |G|\).
    \end{enumerate}
\end{theorem} 

\vspace{-0.2cm}
\comentario{Doesn't support summaries, see \cite[pag. 79]{stein}}
...
\begin{theorem}
    \textbf{4.14.} If \(|G| = p^em\), where \((p, m) = 1\), then \(\forall P \leq G\) \(p\)-Sylow, \(|P|= p^e\).  
\end{theorem}

\demo{\([G:P] = [G:N_G(P)][N_G(P):P]\), where \([G:N_G(P)] = n_p \equiv 1 \ (\operatorname{mod} \ p)\) while \([N_G(P):P] = |\nicefrac{N_G(P)}{P}|\) prime to \(p\) by {\scriptsize \(\square\)} \hyperlink{lemma411}{4.11.}. By \(\blacksquare\) \hyperlink{thm211}{2.11. (\emph{Lagrange})} \(|P| = p^k, \ k \leq e\) and \([G:P] = \nicefrac{|G|}{|P|} = p^{e-k}m\), but is prime to \(p\) so \(k=e\).}

\begin{corollary}
    \textbf{4.15.} If \(p^k \) divides \(|G|\), then \(\exists H\leq G : |H| = p^k\). \comentario{Let \(P \leq G\) \(p\)-Sylow, then \(p^k \) divides \(|P|\) and result follows (\textcolor{rojoscuro}{Ex. 4.2})}
\end{corollary}
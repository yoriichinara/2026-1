...
\begin{theorem}
    \textbf{3.5.} \(\alpha \sim \beta \ \leftrightharpoons \ \alpha\) e \(\beta\) tienen misma estructura cíclica.
\end{theorem}
...
\begin{note}
    En el último capítulo será útil saber cuantos elementos hay en cada clase, en una descomposición completa de \(\alpha\) (incluyendo \(1\)-ciclos), tenemos que: 
    \[|\alpha^{S_n}| = \prod_{r=1}^n \frac{n!}{r^{m_r} \cdot m_r !} \ \ \mid \ \  m_r\ \  \leftarrow  \#r\text{-ciclos en }\alpha.\] 
\end{note}
...
\begin{definition}
    \centering \(\{\alpha\in S_n: \alpha \text{ es par}\} =: A_n \lhd S_n \).   
\end{definition}
...
\tcblower
\[\blacktriangleright \ \mathcal{O}(x) = [x] \in \nicefrac{G}{\sim } \text{ e } G_x = C_G(x). \hspace{4cm}\]
\[ \- \hspace{3cm}\blacktriangleright \ \mathcal{O}(H) = \{H^g : g \in G\} \text{ e } G_H = N_G(H). \]
}

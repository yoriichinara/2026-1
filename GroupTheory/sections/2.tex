...
\begin{corollary}
    \textbf{2.4.} Si \(|G| < \infty\), entonces \(S \leq G \ \leftrightharpoons \ \forall s,t \in S, \ st \in S\). \textcolor{gray}{(\(\Leftarrow\)) Si \(s \in S\), entonces \(\langle s\rangle \leq S\), \(\exists m \in \N: s^m = \id\) e \(s^{-1} = s^{m-1} \in S\) }  
\end{corollary}
...
\ejemplo{
    ...
    \end{example} 
    \tcblower 
     \(\blacktriangleright \ \{a \in G : f(a) = \id_{_H}\} =: \ker f\leq G\).\hfill \(\mid \) \hfill \(\blacktriangleright \ \{f(a) : a \in G\} =:\operatorname{Im}f\leq H\). 
}

\hypertarget{thm25}{}
\begin{theorem}
    \textbf{2.5.} Sean \((S_i)_{i\in I} : S_i \leq G, \ \forall i \in I\), entonces \(\bigcap S_i \leq G\). \comentario{Mogollas }
\end{theorem}
...
\begin{theorem}
    \textbf{2.10.} El número de clases a derecha e izquierda \(S\) en \(G\) coincide. \comentario{Muestre que \(f : Sa \mapsto a^{-1} S\) es una biyección bien definida (no depende del representante)}
\end{theorem}
...
\begin{definition}
    \centering \(\displaystyle \varphi(n) := \# \{k\in \N : k < n \text{ e } (k,n) = 1\} = n \cdot \prod_{p \ \mid \  n} \left(1 - \frac{1}{p} \right)\).
\end{definition}
\hypertarget{thm216}{}
\begin{theorem}
...
\ejemplo{
\begin{example}
\centering   \(\ker f\lhd  G \ \mid \ f : G \to H\)  homomorfismo. 
\end{example}
}
...
\begin{corollary}
    \textbf{2.22.} Si \(N \lhd G\), entonces \(\pi: G \to \nicefrac{G}{N}\) tal que \(\pi:  a\mapsto Na\), es homomorfismo sobrejectivo (\emph{proyección natural}) \(: \ker \pi = N\). \comentario{Mogollas}
\end{corollary}

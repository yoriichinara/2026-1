...
\textcolor{gray}{
  ...
  Como \(C = \gamma_\epsilon \cup [\epsilon, R] \cup \gamma_R \cup [iR, i\epsilon]\), vamos analizar cada uma dessas integrais, \(z = x+iy\).   
    ...
    \textcolor{gray}{
      ...
      \[\M(\sin)(z) = \int_0^1\sin(t)t^{z-1} \ dt + \int_1^\infty \sin(t) t^{z-1} \ dt\]
      ...
      Sejam \(\{z\in \C : |\operatorname{Re}(z)|< 1\} =: \Omega\ab \subset \C\) e \(K\subset \Omega\) compacto, então \(\exists \delta > 0 : -1 + \delta \leq \operatorname{Re}(z) \leq 1 - \delta, \ \forall z\in K \). Como \(|\sin(t) t^{z-1}| \leq |t\cdot t^{z-1}| \leq t^{-1 + \delta}, \ t \in (0,1)\), temos   
      \[\int_0^1 |\sin(t)t^{z-1}| \ dt \leq \int_0^1 t^{-1+\delta} \ dt < \infty, \ \forall z\in K\]
      Tambem, para \(1 \ll t \),  \(\left|\int_1^T\sin(t) \ dt\right| \leq 3\) e \(|t^{z-1}| \leq t^{-\delta} \to 0\), pelo critério de Dirichlet\footnote{Eu desconhecia esse resultado, portanto não tou muito ligado com essa parte do argumento, foi sugestão da IA  + \href{https://math.stackexchange.com/questions/141048/dirichlets-test-for-convergence-of-improper-integrals}{(MathExchange)}.},  
      \[\int_1^\infty \sin(t)t^{z-1} \ dt  < \infty, \ \forall z\in K. \]
      Segue que a integral converge uniformemente em compactos dessa faixa. Pra \(t \in \R_+\) fixo o integrando é holomorfa em \(\Omega\), assim pelo \(\blacksquare\) \hyperlink{thm33}{5.4.} \cite[pag. 56]{shakarchi} a integral define uma função holomorfa em \(\Omega\). Agora, \( \ho(\Omega)\ni \M(\sin)(z) = \Gamma(z) \sin(\pi \cdot \nicefrac{z}{2})\) em \(\{z\in \C : 0 < \operatorname{Re}(z)< 1\}\) conjunto com pontos de acumulação, segue do {\scriptsize\(\boxtimes\)} \hyperlink{coro33}{4.9.} \cite[pag. 52]{shakarchi} que as funções são idénticas em \(\Omega\). 
      \vspace{0.2cm}
      \hrule
      \vspace{-0.3cm}
      \begin{align*}
